...
export function checkUnitDefeat(
  ...
  if (target.stats?.health === undefined || target.stats.health > 0) {
    logger.info(`${target.stats?.health} HP remaining. Target not defeated.`);
    return false;
  }
  ...
  if (target.getRole() === PlayerRole.HERO) {
    ...
    logger.info("Hero defeated. Checking for healing options...");
    ...
    if (healingSpell) {
      logger.info("Hero has a healing spell. using it to heal the hero.");
    ...
    if (healingPotion) {
      logger.info("Hero has a healing potion. using it to heal the hero.");
      target.stats.health = 1;
