...
class BoardInitializer {
  ...
  static initializeBoard(): Tile[][] {
    const board: Tile[][] = [];

    for (let i = 0; i < this.rows; i++) {
      const row: Tile[] = [];
      for (let j = 0; j < this.cols; j++) {
        row.push(new Tile("empty"));
      }
      board.push(row);
    }
    return board;
  ...
  static initializeDoors(): WallGrid {
    return {
      horizontal: Array(20).fill(null).map(() => Array(26).fill(false)),
      vertical: Array(19).fill(null).map(() => Array(27).fill(false)),
    };
  }
}

export { BoardInitializer };
