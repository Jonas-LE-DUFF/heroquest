...
function getPlayerByHero(hero: HeroAsJson, players: PlayerAsJson[]): PlayerAsJson | null {
    const player = players.find(p => p.id === hero.controlledByPlayerId);
    return player || null;
}

function getHeroes(heroes: (HeroAsJson | MonsterAsJson)[]): HeroAsJson[] {
    return heroes.filter((entity): entity is HeroAsJson => 'controlledByPlayerId' in entity);
}
...
function getHeroByPlayerId(playerId: string, game: GameAsJson): HeroAsJson | null {
    const heroes = getHeroes(game.gameState.Units);
    for (let entity of heroes) {
        if ('controlledByPlayerId' in entity && entity.controlledByPlayerId === playerId) {
...
export { getPlayerByHero, getHeroes, getPlayerByHeroCategory, getHeroByPlayerId, getPlayerBySocketId };