...
interface ChooseCharacterProps {
  socket: Socket;
  cancelCallback: () => void;
  chooseCallback: (heroCreation: HeroCreationWish) => void;
  game: GameAsJson;
}

const ChooseCharacter: React.FC<ChooseCharacterProps> = ({
  socket,
  cancelCallback,
  chooseCallback,
  game,
  ...
  const [heroCreation, setHeroCreation] = useState<HeroCreationWish>({
    ...
    equipments: modifiedHero
      ? flattenEquipment(modifiedHero.equipment)
      : getHeroBaseEquipment(getAvailableClasses()[0]),
      ...
      <div className="form">
        ...
        <div>
          <button
            ...
            onClick={() => chooseCallback(heroCreation)}
          >
            Sauvegarder les modifications
          </button>
          <button className="warning-button" onClick={() => cancelCallback()}>
            Annuler
          </button>
        </div>
      </div>
